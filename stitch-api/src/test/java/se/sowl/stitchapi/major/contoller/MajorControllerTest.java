...
class MajorControllerTest {
    ...
    class createMajor{
        @Test
        @DisplayName("POST /api/majors/create - 전공 생성 성공")
        @WithMockUser
        void createMajorSuccess() throws Exception{
            //given
            MajorRequest majorRequest = new MajorRequest("컴퓨터공학과");
            MajorResponse majorResponse = new MajorResponse(1L, "컴퓨터공학과", null);

            //when
            when(majorService.createMajor("컴퓨터공학과")).thenReturn(majorResponse);
        ...
        @Test
        @DisplayName("POST /api/majors/create - 중복된 전공명으로 생성 시 실패")
        @WithMockUser
        void createMajorFail() throws Exception{
            //given
            MajorRequest majorRequest = new MajorRequest("컴퓨터공학과");

            //when
            when(majorService.createMajor("컴퓨터공학과")).thenThrow(new IllegalArgumentException("이미 존재하는 전공명입니다."));
            ...
            mockMvc.perform(post("/api/majors/create")
                    ...
                    .andExpect(status().isBadRequest())
                    .andExpect(jsonPath("$.code").value("FAIL"))
                    .andExpect(jsonPath("$.message").value("이미 존재하는 전공명입니다."))
                    .andDo(print());
        }
